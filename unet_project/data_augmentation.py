...
class DataAugmentation:
    ...
    def extend_database(self):
        ...
        for key, val in tqdm(self._input_images_masks.items()):
            ...
            self._output_masks.append(np.squeeze(mask))

            for i in range(0, self._how_many):
                horizontal_flipped_img, horizontal_flipped_mask = self._random_horizontal_flip(img, mask)
                vertical_flipped_img, vertical_flipped_mask = self._random_vertical_flip(horizontal_flipped_img,
                                                                                        horizontal_flipped_mask)
                cropped_img, cropped_mask = self._random_squared_crop(vertical_flipped_img,
                                                                     vertical_flipped_mask)
                rotated_img, rotated_mask = self._random_rotate(cropped_img, cropped_mask)
                resized_img, resized_mask = self._resize(rotated_img, rotated_mask)
