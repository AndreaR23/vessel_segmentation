...
class ProcessResults:
    ...
    @staticmethod
    def find_contours(img, predicted_img, color=(180, 0, 180), thickness=3):
        contours = measure.find_contours(predicted_img, 0.8)
        pts = contours[0].reshape((-1,1,2))
        contours_img = cv2.polylines(img, np.int32(np.flip(pts, 2)), True, color=color, thickness=thickness)
        return contours_img
